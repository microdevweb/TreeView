...
DeclareModule Tree
    ...
    Declare ResizeTree(idTree,X,Y,W,H)
    Declare SelectItemWithoutCallBack(idTree,item,State.b)
    Declare EnableDragMode(idTree)
EndDeclareModule
Module Tree
    ;       EnableExplicit
    UsePNGImageDecoder()
            ...
            With myTree()\myIcon()
                If \SelectOn
                    If (gMouseX>=\X And gMouseX<=\X+\W) And (gMouseY>=\Y And gMouseY<=\Y+\H)
                        ...
                        EndIf
                        myTree()\CurrentValue=Str(myTree()\myItems()\Type)+Chr(10)+Str(myTree()\myItems()\Value)
                        ProcedureReturn #True
                    EndIf
                EndIf
            EndWith
    ...
    Procedure Event()
        ...
        Select EventType()
            ...
            Case #PB_EventType_MouseMove
                If ClicOn
                    If myTree()\CurrentValue<>""
                        DragText(myTree()\CurrentValue)
                        ProcedureReturn 
                    EndIf
                    ProcedureReturn 
                EndIf
...
;                 SendMessage_(gIdBalloon, #TTM_ACTIVATE,0,0)
    ...
    Procedure Create(id,X.i,Y.i,W.i,H.i)
        If id=#PB_Any
            ...
            Wend
        EndIf
        If FindMap(id,#False):ProcedureReturn -1:EndIf
    ...
    Procedure SelectItem(idTree,item,State.b)
        ...
        CallBackSelected()
    EndProcedure
    Procedure SelectItemWithoutCallBack(idTree,item,State.b)
        If Not FindMap(idTree):ProcedureReturn #False:EndIf 
        DeselectAllItem(idTree)
        If SelectElement(myTree()\myItems(),item)=0
            MessageRequester(#Title,"Error this item "+item+" not exist...")
    ...
    Procedure DisableModeSelect(idTree)
        ...
        EndWith
    EndProcedure
    Procedure ResizeTree(idTree,X,Y,W,H)
        If Not FindMap(idTree):ProcedureReturn #False:EndIf 
        With myTree()
            ResizeGadget(\idScrollArea,X,Y,W,H-30)
            If GadgetWidth(\idCanvas)>W
                SetGadgetAttribute(\idScrollArea,#PB_ScrollArea_InnerWidth,GadgetWidth(\idCanvas))
            Else
                SetGadgetAttribute(\idScrollArea,#PB_ScrollArea_InnerWidth,0)
            EndIf
            If GadgetHeight(\idCanvas)>(H-30)
                SetGadgetAttribute(\idScrollArea,#PB_ScrollArea_InnerHeight,GadgetHeight(\idCanvas))
            Else
                SetGadgetAttribute(\idScrollArea,#PB_ScrollArea_InnerHeight,0)
            EndIf
        EndWith
    EndProcedure
    Procedure EnableDragMode(idTree)
        With myTree()
            If Not FindMap(idTree):ProcedureReturn #False:EndIf 
            \DragModeOn=#True
        EndWith
